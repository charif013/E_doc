...
class ReviewLeaveRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $numbering = $numbering === 'pending_numbering';

        return [
            'is_approved' => ['required', 'boolean'],
            'pin' => ['required_if:is_approved,1', 'nullable', 'string'],
            'reject_reason' => ['required_if:is_approved,0', 'nullable', 'string', 'max:2000'],
            'running_number' => [$numbering ? 'required_if:is_approved,1' : 'nullable', 'integer', 'min:1'],
            'leave_number' => [$numbering ? 'required_if:is_approved,1' : 'nullable', 'string', 'max:100'],
        ];
    }
}
